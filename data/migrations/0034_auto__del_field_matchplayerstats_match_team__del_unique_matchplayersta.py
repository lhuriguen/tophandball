...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'MatchPlayerStats.match_team'
        db.delete_column(u'data_matchplayerstats', 'match_team_id')

        # Adding unique constraint on 'MatchPlayerStats', fields ['match', 'club', 'player']
        db.create_unique(u'data_matchplayerstats', ['match_id', 'club_id', 'player_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'MatchPlayerStats', fields ['match', 'club', 'player']
        db.delete_unique(u'data_matchplayerstats', ['match_id', 'club_id', 'player_id'])


        # User chose to not deal with backwards NULL issues for 'MatchPlayerStats.match_team'
        raise RuntimeError("Cannot reverse this migration. 'MatchPlayerStats.match_team' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'MatchPlayerStats.match_team'
        db.add_column(u'data_matchplayerstats', 'match_team',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['data.MatchTeamStats']),
                      keep_default=False)
    ...
    models = {
        ...
        u'data.matchplayerstats': {
            'Meta': {'unique_together': "(('match', 'club', 'player'),)", 'object_name': 'MatchPlayerStats'},
            'club': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['data.Club']"}),
