...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Delegates & Referees
        self.convert_country(orm.Delegate.objects.all())
        self.convert_country(orm.Referee.objects.all())

    def convert_country(self, queryset):

        # First try to convert to ISO.
        for item in queryset:
            item.country_iso = fields.Country.country_from_ioc(item.country)
            item.save()

        # Handle exceptions to iso.
        for item in queryset.filter(country_iso=''):
            if item.country == 'FAR':
                item.country_iso = 'FO'
                item.save()
            if item.country == 'KOS':
